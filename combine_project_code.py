...
logger = logging.getLogger('linuxsyncpy.combine_project_code')
...
def collect_files(root_dir: Path, current_dir: Path | None = None, output: List[Path] | None = None) -> List[Path]:
    ...
    for entry in entries:
        ...
        logger.debug(f'collect_files inspecting {full_path}')
        if entry.name == ".git":
            logger.debug(f'collect_files skipping git dir {full_path}')
            continue
...
def main() -> int:
    ...
    logger.debug(f'main combining root_dir={root_dir} output_file={output_file}')
    try:
        combine_project_code(root_dir, output_file)
    except Exception as err:
        logger.exception('combine_project_code failed')
