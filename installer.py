...
def download_via_gdown(drive_id: str, output_path: str):
    ...
    if not output_path_obj.is_absolute():
        output_path_obj = Path(tempfile.gettempdir()) / output_path_obj.name
    output_path_obj.parent.mkdir(parents=True, exist_ok=True)
    try:
        code, out, err = run_and_capture(
            f'python -m gdown "{drive_id}" -O "{output_path_obj}"',
            cwd=str(output_path_obj.parent),
        )
        _log_debug(f'gdown returncode={code} stdout={out.strip()!r} stderr={err.strip()!r}')
        logger.debug(
            f'download_via_gdown completed drive_id={drive_id} returncode={code} '
            f'output_exists={output_path_obj.exists()} output_path={output_path_obj}'
        )
        if output_path_obj.exists():
            logger.debug(f'downloaded file exists at {output_path_obj}')
            return str(output_path_obj)
        _log_debug(f'Download did not produce file at {output_path_obj}')
        return None
