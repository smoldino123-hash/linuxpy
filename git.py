...
def _checkout_main_branch(root: Path) -> None:
    logger.debug(f'_checkout_main_branch start root={root}')
    try:
        run_silent('git rev-parse --verify main', cwd=str(root))
        logger.debug('main branch exists locally, checking out main')
        run_silent('git checkout main', cwd=str(root))
        return
    except Exception:
        logger.debug('main branch not present locally')

    try:
        run_silent('git ls-remote --heads origin main', cwd=str(root))
        logger.debug('main branch exists on origin, checking out from origin/main')
        run_silent('git checkout -b main origin/main', cwd=str(root))
        return
    except Exception:
        logger.debug('main branch not present on origin or unresolvable')

    logger.debug('creating new main branch')
    run_silent('git checkout -b main', cwd=str(root))
